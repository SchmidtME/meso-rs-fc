...
for Label = 1:length(Labels)
    ...
    for Layer = 1:length(Layers)
        ...
        for Sbj = 1:length(Sbjs)
            ...
            load([Root,  '/', [Labels{Label}, '_layers_', Layers{Layer}, '_intrahemispheric/'], Sbjs{Sbj},'/CorrelationMtx_Selectivity_subsampled_1.mat']);
            ...
            DATA_ALIKE(Sbj, :, Label, Layer, :, :, :) = Data_Combined(:, 1, :, :, :, 2); % take righ2right, left2left, and z-value

            % Assign the data for the 'Unalike' condition (different eyes)
            DATA_UNALIKE(Sbj, :, Label, Layer, :, :, :) = Data_Combined(:, 4, :, :, :, 2); % take righ2left, and z-value
