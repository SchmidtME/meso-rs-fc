function Data_Combined = A001c_FC_Proc_Data_Interhemi_beta(subName, Root, AnalysisParam, TrgFile)
...
%% Load and show resting-state functional data
    ...
    for q1 = 1:numel(quantile_beta_thresholds)  % Assuming quantile_beta_threshold contains thresholds
        for q2 = 1:numel(quantile_beta_thresholds)
            ...
            % concatenate eye1eye2 and same eye
            [size1_r, size1_c] = size(corr_v1_patch_eye1eye2);
            [size2_r, size2_c] = size(corr_v1_patch_same_eye);
            max_rows = max(size1_r, size2_r);
            max_cols = max(size1_c, size2_c);
            corr_v1_patch_eye1eye2_padded = NaN(max_rows, max_cols);
            corr_v1_patch_eye1eye2_padded(1:size1_r, 1:size1_c) = corr_v1_patch_eye1eye2;
            corr_v1_patch_same_eye_padded = NaN(max_rows, max_cols);
            corr_v1_patch_same_eye_padded(1:size2_r, 1:size2_c) = corr_v1_patch_same_eye;
            corr_v1_patch_both_eyes = [corr_v1_patch_eye1eye2_padded; corr_v1_patch_same_eye_padded];
            

            Both_eyes = mean(corr_v1_patch_both_eyes (:)', 'omitnan');
            Data_Combined(1, q1, q2, 1) = [Both_eyes'];
            % z-transformation
            Both_eyes_z = 0.5 * log((1 + Both_eyes) ./ (1 - Both_eyes));
            Data_Combined(1, q1, q2, 2) = [Both_eyes_z'];
        end
    end
    
    save([Root,'/CorrelationMtx_FC_beta.mat'], 'Data_Combined', 'AnalysisParam') 
