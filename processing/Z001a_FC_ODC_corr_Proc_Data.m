function meanAbsR = Z001a_FC_ODC_corr_Proc_Data(subName, Root, AnalysisParam, TrgFile)
...
% This script calculates the rs-FC for 1000 randomly selected vertices with vertices 
...
% coefficients are averaged and saved as meanAbsR_Denoised_excl_3mm.mat.
...
%% Correlation
% --- For each hemisphere, ring radius, hypothesis and random seed: correlate the ring rs-FC map with the (shifted) ODC map ---
...
for h = 1:numel(hemis)
    ...
    for iDistFromEdge = 1:length(distEdgeRange) % mm
        ...
        for iHypoth = 1:2 % 1 = H1 (same center), 2 = null (spatially shifted center)
            for k = 1:1000
                randInd(1) = ind(randi(length(ind))); % random seed vertex
                if iHypoth == 2 % Null hypothesis
                    randInd(2) = ind(randi(length(ind))); % second, unrelated center for the ODC map
                else
                    randInd(2) = randInd(1); % H1: same center for rs-FC and ODC
                end
                ...
                for circNum = 1:2
                    ...
                    if circNum == 1
                        ...
                        projectedHalton{circNum} = maskedSig(nearestIdx); % map ODC significance onto the ring
                    end
                end
                ...
                % correlate the ring rs-FC map with the (shifted) ODC map for this seed
...
save([Root,'/meanAbsR_Denoised_excl_3mm.mat'], 'meanAbsR', 'distEdgeRange', 'AnalysisParam')
