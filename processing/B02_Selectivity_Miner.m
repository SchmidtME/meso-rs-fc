...
AnalysisParam.Type='intrahemispheric_odc_sep_depths'; % intrahemispheric = within one hemisphere; interhemispheric = between hemispheres
    ...
    for i=1:length(Sbjs)
        ...
        if ~sub2sub
            ...
            if strcmp(AnalysisParam.Type, 'intrahemispheric')
                ...
                for s=1
                    B001a_Selectivity_Proc_Data(Sbjs{i}, Root, AnalysisParam, TrgFile)
                end
            elseif strcmp(AnalysisParam.Type, 'intrahemispheric_1st_run')
                % within V1 or V1 subregions and wthin one hemisphere
                for s=1
                    B001a_Selectivity_Proc_Data_1st_run(Sbjs{i}, Root, AnalysisParam, TrgFile)
                end
            elseif strcmp(AnalysisParam.Type, 'intrahemispheric_last_run')
                % within V1 or V1 subregions and wthin one hemisphere
                for s=1
                    B001a_Selectivity_Proc_Data_last_run(Sbjs{i}, Root, AnalysisParam, TrgFile)
                end
            elseif strcmp(AnalysisParam.Type, 'intrahemispheric_odc_sep_depths')
                % within V1 or V1 subregions and wthin one hemisphere
                for s=1
                    B001a_Selectivity_Proc_Data_odc_sep_depths(Sbjs{i}, Root, AnalysisParam, TrgFile, s, layers{layer})
                end
        ...
        elseif sub2sub
            ...
            B001b_Selectivity_Proc_Data_sub2sub(Sbjs{i}, Root, AnalysisParam, TrgFile)
        end
