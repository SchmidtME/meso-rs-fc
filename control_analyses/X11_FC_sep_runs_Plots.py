...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 27 03:47:28 2025
...
if save_figures:
    ...
    save_dir = f'/space/ardebil/1/users/Others/Marianna/FC_7T_Coronal/Controls/Figures/Figure_2/Suppl/{date_stamp}'
    os.makedirs(save_dir, exist_ok=True)

subjectFolders = ['haas', 'chss', 'aroo', 'aman', 'ylri', 'rcgr', 'atib', 'evad', 'imyy', 'auil'] # arak excluded just one run
ROI = 'V1'
baseDir = '/space/ardebil/1/users/Others/Marianna/FC_7T_Coronal/Controls/Results'

layers = [f'{ROI}_layers_0-10_intrahemispheric_last_run']
...
normalization = 'none' # weighted (alike + unalike), not_weighted (overall FC), none
...
x = np.arange(avg_values_alike.shape[1]) * 2.7  # Adjust spacing for alike/unalike plot
x_diff = np.arange(avg_values_diff.shape[1]) * 2  # Adjust spacing for selectivity plot
...
ax1.errorbar(positions_mid_alike, avg_values_alike[0, :], yerr=std_values_alike[0, :], fmt='none', 
             color='black', capsize=cap_width, elinewidth=2, zorder=0)
deep_alike_bar = ax1.bar(positions_mid_alike, avg_values_alike[0, :], bar_width, 
                         label='Alike', color='white', edgecolor='black', linewidth=1.5)

ax1.errorbar(positions_mid_unalike, avg_values_unalike[0, :], yerr=std_values_unalike[0, :], fmt='none', 
             color='black', capsize=cap_width, elinewidth=2, zorder=0)
deep_unalike_bar = ax1.bar(positions_mid_unalike, avg_values_unalike[0, :], bar_width, 
...
ax1.set_yticks([ylims[0], ylims[1]/2, ylims[1]])

# Selectivity Subplot (right)
ax2 = fig.add_subplot(gs[1])
x_diff = np.arange(avg_values_diff.shape[1]) * 1.6
positions_deep_alike = x_diff - 1 * bar_width

ax2.errorbar(positions_deep_alike, avg_values_diff[0, :], yerr=std_values_diff[0, :], fmt='none', 
             color='black', capsize=cap_width, elinewidth=2, zorder=0)
deep_diff_bar = ax2.bar(positions_deep_alike, avg_values_diff[0, :], bar_width, 
...
ax2.set_yticks([ylims_sel[0], ylims_sel[1]/2, ylims_sel[1]])
...
if save_figures:
    file_name = f"Figure_{layers}.tiff"
...
if save_figures:
    ...
    ax2.errorbar(positions_deep_alike, avg_values_diff[0, :], yerr=std_values_diff[0, :], 
    fmt='none', ecolor='black', elinewidth=3, capsize=cap_width, capthick=3,zorder=0)
    deep_diff_bar = ax2.bar(positions_deep_alike, avg_values_diff[0, :], bar_width, color=cud_colors[0], ecolor='black', edgecolor='black', linewidth=2)
    ...
    plt.tight_layout()
    file_name = f"Figure_clean_{layers}.tiff"
    file_path = os.path.join(save_dir, file_name)
    plt.savefig(file_path, format='tiff', dpi=200)
...
ax2.errorbar(positions_mid_diff, avg_values_diff[0, :], yerr=std_values_diff[0, :], 
             fmt='none', color='black', capsize=cap_width, elinewidth=2, zorder=0)
diff_bar = ax2.bar(positions_mid_diff, avg_values_diff[0, :], width=bar_width, color=cud_colors[0], zorder=1)
...
if save_figures:
    file_name = f"Figure_Suppl_{layers}.tiff"
    file_path = os.path.join(save_dir, file_name)
    plt.savefig(file_path, format='tiff', dpi=200)
...
# # alike and unalike version with labels

# # Define accessible colors
# bar_width = 1  # Width of the bars
# cap_width = 5  # Cap size of error bars

# # Add an explicit offset to increase spacing from the y-axis
# x = np.arange(avg_values_alike.shape[1]) * 2.7 # Add offset directly to x positions

# plt.figure(figsize=(7, 6))

# # Bar positions for deep, mid, and superficial alike and unalike
# positions_deep_alike = x - 1.5 * bar_width
# positions_deep_unalike = x - 0.5 * bar_width
# positions_mid_alike = x + 0.5 * bar_width
# positions_mid_unalike = x + 1.5 * bar_width

# # Plotting Alike (solid colors) with error bars
# plt.errorbar(positions_mid_alike, avg_values_alike[0, :], 
#              yerr=std_values_alike[0, :], fmt='none', color='black', capsize=cap_width, zorder=0)
# deep_alike_bar = plt.bar(positions_mid_alike, avg_values_alike[0, :], bar_width, 
#                          label='Alike', color='white', edgecolor='black', linewidth=1.5)

# # Plotting Unalike (lighter colors) with error bars
# plt.errorbar(positions_mid_unalike, avg_values_unalike[0, :], 
#              yerr=std_values_unalike[0, :], fmt='none', color='black', capsize=cap_width, zorder=0)
# deep_unalike_bar = plt.bar(positions_mid_unalike, avg_values_unalike[0, :], bar_width, 
#                            label='Unalike', color='black', edgecolor='black', linewidth=1.5)


# # X and Y labels, title, and ticks
# mid_x_ticks = (positions_mid_alike + positions_mid_unalike) / 2
# plt.xticks(ticks=mid_x_ticks, labels=range(1, 11))  # Replace with your distance labels
# plt.xlabel('Distance')
# plt.ylim(ylims)

# if fisher_transform == 1:
#     plt.ylabel('z-transformed Pearson correlation coefficient')
# else:
#     plt.ylabel('Pearson correlation coefficient')

# plt.title('Effect of cortical depth and distance on rs-FC strength (Alike vs Unalike)')
# plt.yticks(ticks=[ylims[0], ylims[0] + (ylims[1] - ylims[0]) / 4, 
#                   ylims[0] + (ylims[1] - ylims[0]) / 2, 
#                   ylims[0] + 3 * ((ylims[1] - ylims[0]) / 4), ylims[1]])

# # Reduce padding in the layout
# plt.tight_layout()
# plt.legend(handles=[deep_alike_bar, deep_unalike_bar])
# plt.show()

# #%% selectiviy version with labels

# # Define accessible colors
# cud_colors = ['#808080']
# bar_width = 1  # Width of the bars
# cap_width = 5 # cap size of errorbars

# x = np.arange(avg_values_diff.shape[1]) * 2  # Adjust spacing between distances
...
# # Plotting Alike (solid colors) with error bars first
# #deep
# plt.errorbar(positions_deep_alike, avg_values_diff[0, :], 
#              yerr=std_values_diff[0, :], fmt='none', color='black', capsize=cap_width, zorder=0)
# deep_alike_bar = plt.bar(positions_deep_alike, avg_values_diff[0, :], bar_width, 
#                          label='Selectivity', color=cud_colors[0], edgecolor='black', linewidth=1.5)


# plt.xlim(left=positions_deep_alike[0]-0.25 - bar_width * 0.5)
# mid_x_ticks = positions_deep_alike
# plt.xticks(ticks=mid_x_ticks, labels=range(1, 11))  # Replace with your distance labels
# plt.xlabel('Distance')
# plt.ylim(bottom=0)
# if fisher_transform == 1:
#     if normalization == 'weighted':
#         plt.ylabel('% change of z-transformed Pearson r (weighted normalization)')
#     elif normalization == 'not_weighted':
#         plt.ylabel('% change of z-transformed Pearson r (normalization)')
#     else: 
#         plt.ylabel('difference of z-transformed Pearson r')
# else:
#     if normalization == 'weighted':
#         plt.ylabel('% change of Pearson r (weighted normalization)')
#     elif normalization == 'not_weighted':
#         plt.ylabel('% change of Pearson r (normalization)')
#     else: 
#         plt.ylabel('difference of Pearson r')

# plt.title('Effect of cortical depth and distance on selectivity')
# #plt.yticks(ticks=[ylims[0], ylims[0] + (ylims[1]-ylims[0])/4, ylims[0] + (ylims[1]-ylims[0])/2, ylims[0] + 3*((ylims[1]-ylims[0])/4), ylims[1]])

# plt.legend(handles=[deep_alike_bar])
# plt.tight_layout()
# plt.show()  

# np.mean(avg_values_diff[0, :])
# np.std(avg_values_diff[0, :])/np.sqrt(10)