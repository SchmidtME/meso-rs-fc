...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 22 11:57:34 2025
...
for sessionNum, rsSessionName in enumerate(rsSessions, start=1):
    ...
    for trgIdx, trgFile in enumerate(AnalysisParam['TrgFiles']):
        ...
        # Calculate mean across timepoints
...
# Create the original plot for the entire time series (mean across target files for the vertex)
plt.figure(figsize=(15, 8), facecolor='white')  # Set figure background to white
ax = plt.gca()
ax.set_facecolor('white')  # Set plot background to white
colors = ['#0072B2', '#E69F00', '#009E73']  # Updated colors
labels = ['deep', 'middle', 'superficial']

# Plot the mean across vertices for a single vertex
for trgIdx, trgFile in enumerate(AnalysisParam['TrgFiles']):
    plt.plot(x, np.mean(all_rs_v1_patch_4vtx[trgIdx], axis=0), marker='o', linestyle='-', color=colors[trgIdx])

# Add a red transparent rectangle
#plt.axvspan(0, 9, color='red', alpha=0.3)

# Adjusting the axis limits to remove the space before the first timepoint
plt.xlim(0, 131)

# Set specific x-ticks and y-ticks for the main plot, without labels
plt.xticks(ticks=[0, 10, 128], labels=[''] * 3)  # Keep ticks but remove labels
plt.yticks([],labels=[]) 
plt.grid(False)  # Remove gridlines

if save_figures:
    file_name = f"Figure_1E_clean.tiff"
...
# Create a zoomed-in plot for timepoints 50 to 60 (mean across target files for the vertex)
plt.figure(figsize=(15, 8), facecolor='white')  # Set figure background to white
ax = plt.gca()
ax.set_facecolor('white')  # Set plot background to white
...
mean_vertex_time_series = np.mean(np.mean(all_rs_v1_patch_4V1, axis=0), axis=1)  # Mean across vertices and sessions

# Plot the mean time series across target files
for trgIdx, trgFile in enumerate(AnalysisParam['TrgFiles']):
    plt.plot(x, mean_vertex_time_series[trgIdx], marker='o', linestyle='-', color=colors[trgIdx])
...
# Set specific x-ticks and y-ticks for the main plot, without labels
plt.xticks(ticks=[0, 10, 128], labels=[''] * 3)  # Keep ticks but remove labels
plt.yticks([],labels=[])  # Keep y-ticks but remove labels

plt.grid(False)  # Remove gridlines

if save_figures:
    file_name = f"Figure_1D_clean.tiff"
    file_path = os.path.join(save_dir, file_name)
    plt.savefig(file_path, format='tiff', dpi=600)
