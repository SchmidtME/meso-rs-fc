function Data_Combined = B011s_Selectivity_Proc_Data_vis_resp(subName, Root, AnalysisParam, TrgFile,s)
...
selected_indices_eye1eye2 = load(fullfile('/space/ardebil/1/users/Others/Marianna/FC_7T_Coronal/Controls/Results/Supplementary', [AnalysisParam.ROI '_layers_0-10_intrahemispheric_vis_resp'], subName, sprintf('CorrelationMtx_FC_Params_subsampled_%d.mat', s))).AnalysisParam.selected_indices_eye1eye2;
selected_indices_same_eye = load(fullfile('/space/ardebil/1/users/Others/Marianna/FC_7T_Coronal/Controls/Results/Supplementary', [AnalysisParam.ROI '_layers_0-10_intrahemispheric_vis_resp'], subName, sprintf('CorrelationMtx_FC_Params_subsampled_%d.mat', s))).AnalysisParam.selected_indices_same_eye;
% TimeSeries Params
...
for h = 1:numel(hemis)
    ...
    StrtTime = toc;
    % load single condition maps of DE and NDE separately
    dataFile_odc_R = fullfile(odcFolder, [d '.' hemi], '2D_R', [hemi '.ffx.osgm.wls'], 'beta.nii');  
    sig_odc_R{h} = load_nifti(dataFile_odc_R); 
    sig_odc_R{h} = sig_odc_R{h}.vol; 
    dataFile_odc_C = fullfile(odcFolder, [d '.' hemi], '2D_C', [hemi '.ffx.osgm.wls'], 'beta.nii');
    sig_odc_C{h} = load_nifti(dataFile_odc_C); 
    sig_odc_C{h} = sig_odc_C{h}.vol; 
    % Save values that are showingeither the strongest response for left or right eye
    max_abs_value_matrix = zeros(size(sig_odc_R{h}));
    mask = abs(sig_odc_C{h}) > abs(sig_odc_R{h});
    max_abs_value_matrix(mask) = (abs(sig_odc_C{h}(mask)))*-1;
    max_abs_value_matrix(~mask) = abs(sig_odc_R{h}(~mask));
    sig_odc{h} = max_abs_value_matrix;
...
for h = 1:numel(hemis)
     for q1 = 1:numel(quantile_beta_thresholds)  % Assuming quantile_beta_threshold contains thresholds
        for q2 = 1:numel(quantile_beta_thresholds)
            ...
            %selected_indices_eye1eye2 = cell(100, 1);
            %selected_indices_same_eye = cell(100, 1);
            
            for i = 1:100
                %selected_indices_eye1eye2{i} = zeros(2,10,10);
                %selected_indices_same_eye{i} = zeros(2,10,10);
                if quants_number_eye1eye2(i) > quants_number_same_eye(i)
                    %all_indices = 1:numel(dist_v1_patch_eye1eye2_quants{i});
                    %selected_indices_eye1eye2{i}(h,q1,q2,1:quants_number_same_eye(i)) = randsample(all_indices, quants_number_same_eye(i));
                    ...
                    %all_indices = 1:numel(dist_v1_patch_same_eye_quants{i});
                    %selected_indices_same_eye{i}(h,q1,q2,1:quants_number_eye1eye2(i)) = randsample(all_indices, quants_number_eye1eye2(i));
            ...
            quants_corr_v1_patch_eye2eye2{h} = dist_quantiles_10_fin;
            ...
            quants_corr_v1_patch_eye1eye2{h} = dist_quantiles_10_fin;
            mean_quants_corr_v1_patch_eye1eye2 = arrayfun(@(i) mean(corr_v1_patch_eye1eye2_subsampled_concat(dist_v1_patch_eye1eye2_subsampled_concat>quants_corr_v1_patch_eye1eye2{h}(i) & dist_v1_patch_eye1eye2_subsampled_concat<=quants_corr_v1_patch_eye1eye2{h}(i+1)), "omitnan"), 1:nQuant);

            quants_corr_v1_patch_same_eye{h} = dist_quantiles_10_fin;
            ...
            mean_quants_dist_v1_patch_eye1eye2 = arrayfun(@(i) mean(dist_v1_patch_eye1eye2_subsampled_concat(dist_v1_patch_eye1eye2_subsampled_concat>quants_corr_v1_patch_eye1eye2{h}(i) & dist_v1_patch_eye1eye2_subsampled_concat<=quants_corr_v1_patch_eye1eye2{h}(i+1)), "omitnan"), 1:nQuant);
            median_quants_dist_v1_patch_eye1eye2 = arrayfun(@(i) median(dist_v1_patch_eye1eye2_subsampled_concat(dist_v1_patch_eye1eye2_subsampled_concat>quants_corr_v1_patch_eye1eye2{h}(i) & dist_v1_patch_eye1eye2_subsampled_concat<=quants_corr_v1_patch_eye1eye2{h}(i+1)), "omitnan"), 1:nQuant);

            AnalysisParam.same_eye_mean_quants_dist{h}(q1,q2,:) = mean_quants_dist_v1_patch_same_eye;
            AnalysisParam.same_eye_median_quants_dist{h}(q1,q2,:) = median_quants_dist_v1_patch_same_eye;
...
if ~exist(sprintf('%s/CorrelationMtx_Selectivity_subsampled_%d.mat', Root, s))
    save(sprintf('%s/CorrelationMtx_Selectivity_subsampled_%d.mat', Root, s), 'Data_Combined');
...
% if ~exist(sprintf('%s/CorrelationMtx_Selectivity_Params_subsampled_%d.mat', Root, s))
%     save(sprintf('%s/CorrelationMtx_Selectivity_Params_subsampled_%d.mat', Root, s), 'AnalysisParam', '-v7.3');
